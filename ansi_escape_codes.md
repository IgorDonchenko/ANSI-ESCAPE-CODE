...
# <a name="CoursorNavigation"></a> Очистить экран
...
```
В результате получим абрукадабру. Одна надпись наложилась поверх другой, но не затерла её полностью:

```
Вторая надписьпись на экране
```

Эту проблему можно решить добавлением пробелов, привести все строки к одной ширине:
...
Это не очень удобно: сложно менять текст сообщений и легко ошибиться с количеством пробелов. Если вы можете себе позволить очистить экран полностью, то воспользуйтесь спец.символом `'\033[2J'`:
